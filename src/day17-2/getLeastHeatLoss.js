...
const getNextPoints = (point, matrix, closedPointsMap) => {
  ...
  const cannotTurn = point.step < MIN_STEPS && point.direction;
  ...
  if (
    ...
    (cannotTurn ? point.direction === Directions.right : true)
  ) {
  ...
  if (
    ...
    (cannotTurn ? point.direction === Directions.left : true)
  ) {
  ...
  if (
    ...
    (cannotTurn ? point.direction === Directions.bottom : true)
  ) {
  ...
  if (
    ...
    (cannotTurn ? point.direction === Directions.top : true)
  ) {
...
export const getLeastHeatLoss = (matrix) => {
  ...
  for (let i = MIN_STEPS; i < MAX_STEPS + 1; i++) {
    const c1 =
